...
var m = [60, 0, 10, 0],
    ...
    //       within: function(d, extent, dim) { return extent[0] <= dim.scale(d) && dim.scale(d) <= extent[1]; },
    //       defaultScale: d3.scale.ordinal().range([0, innerHeight])
    //     }
    //   };
...
//  var mut_name="Chromosome"+d.mutation.chromosome;    

var colors = {
  "single base substitution": [204,70,41],
  "insertion of <=200bp": [359,69,49]
};
...
d3.csv("data/mutations.csv", function(raw_data) {
  ...
  var g = svg.selectAll("#chart .dimension")
      .data(dimensions)
    .enter().append("svg:g")
      ...
      .call(d3.behavior.drag()
        .on("dragstart", function(d) {
          ...
          d3.select("#chart #foreground").style("opacity", "0.35");
        })
        .on("drag", function(d) {
        ...
        .on("dragend", function(d) {
          ...
          // rerender
          d3.select("#chart #foreground").style("opacity", null);
          brush();
...
function create_legend(colors,brush) {
  ...
  var legend = legend_data
    .enter().append("div")
      .attr("title", "Hide Mutation Type")
      .on("click", function(d) { 
        // toggle mutation type
        if (_.contains(excluded_types, d)) {
          d3.select(this).attr("title", "Hide group")
          excluded_types = _.difference(excluded_types,[d]);
          brush();
        } else {
          d3.select(this).attr("title", "Show group")
          excluded_types.push(d);
          brush();
        }
      });
...
function render_range(selection, i, max, opacity) {
  selection.slice(i,max).forEach(function(d) {
    path(d, foreground, color(d.type,opacity));
  });
};
...
function data_table(sample) {
  ...
  var table = d3.select("#mutation-list")
    .html("")
    .selectAll(".row")
      .data(sample)
    .enter().append("div")
  ...
  table
    .append("span")
      ...
      .style("background", function(d) { return color(d.type,0.85) })
...
function highlight(d) {
  ...
  d3.selectAll(".row").style("opacity", function(p) { return (d.type == p) ? null : "0.3" });
  path(d, highlighted, color(d.type,1));
}
...
function brush() {
  ...
  data
    .filter(function(d) {
      return !_.contains(excluded_types, d.type);
    })
    .map(function(d) {
  ...
  var tallies = _(selected)
    .groupBy(function(d) { return d.type; })

  // include empty groups
  _(colors).each(function(v,k) { tallies[k] = tallies[k] || []; });

  legend
    .style("text-decoration", function(d) { return _.contains(excluded_types,d) ? "line-through" : null; })
